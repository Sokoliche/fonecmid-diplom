...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ФормированиеДвиженийОклад() Тогда 
		Отказ = Истина;
	КонецЕсли;
...
Функция ФормированиеДвиженийОклад()
	...
	Пока Выборка.Следующий() Цикл 
		...
		Если Выборка.Оклад = 0 Тогда
			...
			Текст = СтрШаблон("У сотрудника %1 не установлен оклад, для расчета зарплаты установите оклад не меньше 1!", Выборка.Сотрудник);	
		...
		Иначе 
			...
			Если Выборка.СуммаВыпРаб > 0 Тогда 
				...
				//Движение регистра расчета Основные начисления
				...
				ДвижениеДопВып.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.ПроцентЗаРаботы;
				...
				ДвижениеДопВып.Процент = Выборка.ПроцентОтРабот;
				ДвижениеДопВып.Результат = Выборка.СуммаВыпРаб;			
				...
				ДвижениеНДФЛ.ВидУдержания = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.ПроцентЗаРаботы;
...
Процедура РасчетОклада()
	...
	|	ВКМ_ОсновныеНачисленияДанныеГрафика.Результат КАК СуммаВыпРаб
	|ИЗ
...
Процедура РасчетОтпуска()
	...
	|		ПО ВременнаяТаблица.Сотрудник = ВКМ_УсловияОплатыСотрудниковСрезПоследних.Сотрудник";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (Выборка.ОкладБаза + Выборка.ПремияБаза) = 0 Тогда
			
			СуммаДоВычетаНДФЛ = Выборка.Оклад; 
